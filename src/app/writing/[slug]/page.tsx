...
import { defaultFilters, filterWriting, writingPeriods } from '@/lib/content/writing-model'
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!entry) notFound()
  return buildPageMetadata({ title: entry.title, description: `${entry.title} · ${entry.type === 'diary' ? '日记' : '随笔'} · 文字`, path: `/writing/${entry.slug}` })
}
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    <article lang={entry.lang}>
      <header className="writing-article-heading"><h1>{entry.title}</h1><p className="sr-only">{entry.date ? <time dateTime={entry.date}>原档日期 {entry.date.replaceAll('-', '.')}</time> : '日期未注明'}<span aria-hidden="true"> · </span>{entry.type === 'diary' ? '日记' : '随笔'}</p></header>
