...
test('year and month archive, chronological filters, history and article return', async ({ page }) => {
  ...
  await expect(page.locator('.writing-month')).toHaveCount(new Set(published.map(entry => entry.date?.slice(0, 7) ?? 'unknown')).size)
  expect(await page.locator('.writing-entry-title').allTextContents()).toEqual(ordered.map(entry => entry.title))
  await expect(page.locator('.writing-archive-title')).toHaveText('2026年 · 九月')
  await expect(page.locator('.writing-month[data-month="2025-11"] .writing-month-heading')).toHaveText('2025十一月')
  await expect(page.locator('.writing-month[data-month="2020-11"] .writing-entry')).toHaveCount(1)
  ...
  await page.getByText('筛选文字', { exact: true }).click()
  await page.getByLabel('排序', { exact: true }).selectOption('oldest')
  await expect(page.locator('.writing-archive-title')).toHaveText('2020年 · 十一月')
...
test('responsive layout, actual Chinese font, accessibility and other modules', async ({ page }, testInfo) => {
  ...
  for (const width of [320, 390, 768, 1440, 2504]) {
    ...
    await expect(page.locator('.writing-month-heading').first()).toHaveText('八月')
